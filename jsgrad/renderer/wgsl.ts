...
export class WGSLRenderer extends CStyleLanguage {
  ...
  override string_rewrite = new PatternMatcher<WGSLRenderer, string | undefined>([
    ...
    UPat.index(UPat.var('b'), UPat.var('idx')).fn(({ ctx, b, idx }) => {
      ...
      const x = ctx.bufs!.get(b)
      // TODO: this shoud probably be in extra_matcher
      return !x || is_storage(x.dtype, x.mutable) ? `${data}[${index}]` : `${data}[${index} / 4][${index} % 4]`
    }),
  ...
  override render_kernel = ({ function_name, kernel, uops }: RenderKernelArgs): string => {
    ...
    prg += [
      ...external_local_bufs,
      ...[...this.bufs!.entries()].map(([_, { name, dtype, mutable }], i) =>
        `@group(0) @binding(${i + 1})` +
